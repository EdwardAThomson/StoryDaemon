...
7.6. `_control_scene_tension()` dispatches one bounded correction, never both: on a normal tick `_maybe_rewrite_for_tension()`, which for a scene written to a paragraph plan revises **only the tension-carrying paragraphs** (`agent/partial_revision.py` picks them from the corpus's measured tension-band shares: ACTION rises 1.33x with tension where LORE falls to 0.56x) and returns every other paragraph byte-identical, so the plan, the scene's length and its marker compliance survive the pass; a whole-scene rewrite destroyed all three live on 2026-09-12 (paragraph counts fell 64 to 36 and 57 to 28). Capped by `coherence.partial_rewrite_max_blocks` and never more than a third of a scene. **Structure is enforced in code, not requested in the prompt:** the parser re-joins a block the model split across blank lines into its marked owner (rather than dropping the tail, which silently deleted prose) and collapses internal line breaks, `check_lengths` rejects a block returned at over 3x or under a quarter of the paragraph it replaces (bounded against the page, not the plan, since the writer already lands under the plan), and `splice` ignores indices that were not selected or fall outside the scene. Unplanned scenes keep the original whole-scene path (if the scored scene is > `coherence.tension_rewrite_threshold` off the target, one context-rich prose revision toward it, kept only if closer; skipped as futile when the gap exceeds what prose can close (`rewrite_futile`, gated with `coherence.arc_phase_mandate`)); on the finale tick `_finale_reroll_for_tension()` instead, up to `coherence.finale_retries` *fresh* renders against the finale tension cap (the hot-finale flips are staging choices a rewrite cannot unstage). Never raises.
...
**Sectioned writing (DSL Slice 5, `generation.subblock_generation`, default off)** writes the scene across several plan-addressed calls instead of one, because a single request will not reliably produce a masters-length chapter (corpus median 3,165 words). `segments.py:partition_skeleton` splits the plan into contiguous `[n]` ranges (`subblock_section_blocks`, default 10; set it to 1 for the landing sketch's literal per-sub-block mode), a trailing stub merges into its predecessor so the ending is never written by a one-block call, and `section_word_target` sizes each call from its own mode mix. `SceneWriter._write_in_sections` drives it: every call gets the whole brief, all prose so far, and only its slice of the plan, with first/middle/last position instructions so only the final section lands the ending. The plan rules (`scene_skeleton.py:plan_rules`) are shared with the single-shot prompt rather than duplicated, since a drifted copy would silently undo the marker protocol or the dialogue-turn rule. Requires `enable_scene_skeleton`, since the plan is what addresses the sections; **any failure returns None and the caller writes single-shot exactly as before**, so the flag can cost quality but never a scene.
