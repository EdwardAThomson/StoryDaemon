...
7.6. `_control_scene_tension()` dispatches one bounded correction, never both: on a normal tick `_maybe_rewrite_for_tension()`, which for a scene written to a paragraph plan revises **only the tension-carrying paragraphs** (`agent/partial_revision.py` picks them from the corpus's measured tension-band shares: ACTION rises 1.33x with tension where LORE falls to 0.56x) and returns every other paragraph byte-identical, so the plan, the scene's length and its marker compliance survive the pass; a whole-scene rewrite destroyed all three live on 2026-09-12 (paragraph counts fell 64 to 36 and 57 to 28). Capped by `coherence.partial_rewrite_max_blocks` and never more than a third of a scene. Unplanned scenes keep the original whole-scene path (if the scored scene is > `coherence.tension_rewrite_threshold` off the target, one context-rich prose revision toward it, kept only if closer; skipped as futile when the gap exceeds what prose can close (`rewrite_futile`, gated with `coherence.arc_phase_mandate`)); on the finale tick `_finale_reroll_for_tension()` instead, up to `coherence.finale_retries` *fresh* renders against the finale tension cap (the hot-finale flips are staging choices a rewrite cannot unstage). Never raises.
