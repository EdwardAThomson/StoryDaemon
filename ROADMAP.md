...
- [ ] Gate A re-run against a sampler with `_SCENE_TYPES` removed (second-order kernel plus measured openers/closers only). Section 12 of the study finds the scene layer earns none of the return rate and costs a little accuracy on shares and run lengths, but Gate A's 25/25 was scored against the hybrid, so the comparison has to be made on the same instrument before anything is removed from a shipping path
